...
const redirect_uri = `${FRONT_USER_HOST}/member/kakao`;
...
export const getAccessToken = async (authCode) => {
  try {
    const res = await axios.get(`${API_URL}/api/member/kakao/token?code=${authCode}`);
...
export const getMemberWithAccessToken = async (accessToken) => {
  ...
  try {
    const res = await axios.get(`${API_URL}/api/member/kakao?accessToken=${accessToken}`, headers);
